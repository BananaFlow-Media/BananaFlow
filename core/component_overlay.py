...
def parse_manifest(raw: bytes, *, app_version: str = FULL_VERSION) -> ComponentManifest:
    ...
    if not bundle_id or any(ch not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789._-" for ch in bundle_id):
    ...
    if not _version_at_least(app_version, minimum) or not _version_before(app_version, maximum):
...
def _valid_bundle(root: Path, bundle_id: object) -> Path | None:
...
def activate_component_overlay(*, root: Path | None = None) -> Path | None:
    ...
    selected = _valid_bundle(target_root, active_id)
    selected_id = active_id
    if selected is None:
        selected = _valid_bundle(target_root, previous_id)
