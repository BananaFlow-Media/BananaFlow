...
- Clarified current platform support: Windows 10/11 x64 and macOS Apple Silicon are supported packaged targets; Linux is supported from source even though no official Linux installer/package is published yet.
...
* **Packaging**: Windows installer and portable package; macOS arm64 package; SHA-256 checksums; CycloneDX SBOM; GitHub build-attestation evidence.
...
* **macOS signing/notarization is limited**, so the supported package can require the documented Gatekeeper first-run approval.
* **Linux has no official installer/package yet**; supported use is currently from source.
