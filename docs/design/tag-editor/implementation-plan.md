...
### שינוי התנהגות מכוון (C2)
היום ה־recursive הוא בחירת משתמש ב־`_on_browse` (checkbox בדיאלוג הבחירה). לפי הכרעתך המתג מוסר לחלוטין והסריקה תמיד רקורסיבית.
**זו הסרה מכוונת של אפשרות משתמש קיימת, לא פער שנשכח** — מתועד כאן במפורש. ה־signal `scan_requested(Path, bool)` ימשיך לקבל את הפרמטר השני (לשמור תאימות עם ה־Controller) אך תמיד יישלח `True`.
